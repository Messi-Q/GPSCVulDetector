...
class CGEConv:
    def __init__(self, graph_train, graph_test, pattern_train, pattern_test, y_train, y_test,
        ...
        # pattern_train = tf.keras.layers.Conv1D(4, kernel_size=3, strides=1, activation=tf.nn.relu, padding='same')(
        #     input2)
        ...
        Dense1 = tf.keras.layers.Dense(200, activation='relu')(input1)
        Dense2 = tf.keras.layers.Dense(100, activation='relu')(Dense1)
        prediction = tf.keras.layers.Dense(1, activation='sigmoid', name='output')(Dense2)

        model = tf.keras.Model(inputs=input1, outputs=[prediction])
    ...
    def train(self):
        self.model.fit(self.graph_train, self.y_train, batch_size=self.batch_size,
                       epochs=self.epochs,
    ...
    def test(self):
        ...
        values = self.model.evaluate(self.graph_test, self.y_test, batch_size=self.batch_size,
                                     verbose=1)
        print("Loss: ", values[0], "Accuracy: ", values[1])
        predictions = (self.model.predict(self.graph_test, batch_size=self.batch_size).round())
